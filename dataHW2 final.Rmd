...
colors <- rainbow(nrow(mushroom_x))

# optimal lambda
optimal_lambda = log(mushroom_lasso$lambda[which.min(AICc(mushroom_lasso))])
optimal_lambda

plot(mushroom_lasso, main="LASSO Coefficient Paths", xlab="Log Lambda", ylab="Coefficient",
     col=colors, lwd=0.5)+
abline(v=optimal_lambda, col="black", lwd=2, lty=2)+
text(x=optimal_lambda, y=par("usr")[3], labels=paste("Optimal lambda:\n", round(optimal_lambda, 2)), 
...
conf_mushroom = table(y = mushroom_test$class, 
                           yhat = yhat_mushroom)
conf_mushroom
TPR <- conf_mushroom[2, 2] / sum(conf_mushroom[2, ])
FPR <- conf_mushroom[1, 2] / sum(conf_mushroom[1, ])

cat("True Positive Rate (Sensitivity):", TPR, "\n")
cat("False Positive Rate (1 - Specificity):", FPR, "\n")
